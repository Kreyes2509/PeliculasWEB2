...
class validarRol
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if(auth()->check() && auth()->user()->status_code && auth()->user()->status_qr && auth()->user()->rol_id  == 1 && auth()->user()->status  == 1)
        {
        ...
        if(auth()->check() && auth()->user()->status_code && auth()->user()->rol_id  == 2 && auth()->user()->status  == 1)
        {
            return $next($request);
        }
